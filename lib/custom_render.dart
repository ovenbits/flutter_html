...
CustomRenderMatcher blockElementMatcher() => (context) {
      return context.tree.style.display == Display.BLOCK && (context.tree.children.isNotEmpty || context.tree.element?.localName == "hr");
    };
...
CustomRenderMatcher dataUriMatcher({String? encoding = 'base64', String? mime}) => (context) {
      ...
      return dataUri != null && dataUri.namedGroup('mime') != "image/svg+xml" && (mime == null || dataUri.namedGroup('mime') == mime) && (encoding == null || dataUri.namedGroup('encoding') == ';$encoding');
    };
...
CustomRenderMatcher assetUriMatcher() => (context) => context.tree.element?.attributes.cast() != null && _src(context.tree.element!.attributes.cast()) != null && _src(context.tree.element!.attributes.cast())!.startsWith("asset:") && !_src(context.tree.element!.attributes.cast())!.endsWith(".svg");
...
CustomRender blockElementRender({Style? style, List<InlineSpan>? children}) => CustomRender.inlineSpan(inlineSpan: (context, buildChildren) {
          ...
          children: (children as List<TextSpan>?) ??
              context.tree.children
                  .expandIndexed((i, childTree) => [
                        ...
                        if (i != context.tree.children.length - 1 && childTree.style.display == Display.BLOCK && childTree.element?.localName != "html" && childTree.element?.localName != "body") TextSpan(text: "\n"),
                      ])
                  .toList(),
        );
      }
        ...
        children: children ??
            context.tree.children
                .expandIndexed((i, childTree) => [
                      ...
                      context.parser.parseTree(context, childTree),
                      if (context.parser.shrinkWrap && i != context.tree.children.length - 1 && childTree.style.display == Display.BLOCK && childTree.element?.localName != "html" && childTree.element?.localName != "body") TextSpan(text: "\n"),
                    ])
                .toList(),
      ));
    });
...
CustomRender replacedElementRender({PlaceholderAlignment? alignment, TextBaseline? baseline, Widget? child}) => CustomRender.inlineSpan(
    inlineSpan: (context, buildChildren) => WidgetSpan(
...
CustomRender textContentElementRender({String? text}) => CustomRender.inlineSpan(inlineSpan: (context, buildChildren) => TextSpan(text: (text ?? (context.tree as TextContentElement).text).transformed(context.tree.style.textTransform)));
          ...
          builder: (buildContext) {
            return GestureDetector(
              ...
              onTap: () {
                ...
                context.parser.onImageTap?.call(_src(context.tree.element!.attributes.cast())!.split("base64,")[1].trim(), context, context.tree.element!.attributes.cast(), context.tree.element);
              },
            );
          });
    });
          ...
          builder: (buildContext) {
            return GestureDetector(
              ...
              onTap: () {
                ...
                context.parser.onImageTap?.call(assetPath, context, context.tree.element!.attributes.cast(), context.tree.element);
              },
            );
          });
    });
      ...
      final widget = FutureBuilder<Size>(
        ...
        builder: (BuildContext buildContext, AsyncSnapshot<Size> snapshot) {
          if (snapshot.hasData) {
            return Container(
              ...
              child: AspectRatio(
                ...
                child: Image.network(
                  ...
                  frameBuilder: (ctx, child, frame, _) {
                    if (frame == null) {
                      return altWidget?.call(_alt(attributes)) ?? Text(_alt(attributes) ?? "", style: context.style.generateTextStyle(context.buildContext));
                    }
                    return child;
                  },
                ),
            ...
            return altWidget?.call(_alt(context.tree.element!.attributes.cast())) ?? Text(_alt(context.tree.element!.attributes.cast()) ?? "", style: context.style.generateTextStyle(context.buildContext));
          } else {
...
CustomRender interactableElementRender({List<InlineSpan>? children}) => CustomRender.inlineSpan(
    inlineSpan: (context, buildChildren) => TextSpan(
          children: children ??
              (context.tree as InteractableElement).children.map((tree) => context.parser.parseTree(context, tree)).map((childSpan) {
                return _getInteractableChildren(context, context.tree as InteractableElement, childSpan, context.style.generateTextStyle(context.buildContext).merge(childSpan.style));
              }).toList(),
        ));
  ...
  if (childSpan is TextSpan) {
    ...
    return WidgetSpan(
      child: MultipleTapGestureDetector(
        onTap: context.parser.internalOnAnchorTap != null ? () => context.parser.internalOnAnchorTap!(tree.href, context, tree.attributes, tree.element) : null,
