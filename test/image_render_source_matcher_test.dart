...
void main() {
  ...
  group("custom image data uri matcher", () {
    CustomRenderMatcher matcher = dataUriMatcher(encoding: null, mime: 'image/svg+xml');
    test("matches an svg data uri with base64 encoding", () {
      expect(_match(matcher, 'data:image/svg+xml;base64,PD94bWwgdmVyc2lvbj0iMS4wIiBlbmNvZGluZz0iVVRGLTgiPz4KPHN2ZyB2aWV3Qm94PSIwIDAgMzAgMjAiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+CjxjaXJjbGUgY3g9IjE1IiBjeT0iMTAiIHI9IjEwIiBmaWxsPSJncmVlbiIvPgo8L3N2Zz4='), isTrue);
    });
    test("matches an svg data uri without specified encoding", () {
      expect(_match(matcher, 'data:image/svg+xml,%3C?xml version="1.0" encoding="UTF-8"?%3E%3Csvg viewBox="0 0 30 20" xmlns="http://www.w3.org/2000/svg"%3E%3Ccircle cx="15" cy="10" r="10" fill="green"/%3E%3C/svg%3E'), isTrue);
    });
    test("matches base64 data uri without data", () {
      expect(_match(matcher, 'data:image/svg+xml;base64,'), isTrue);
    });
    test("doesn't match non-base64 image data uri", () {
      expect(_match(matcher, 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAUAAAAFCAYAAACNbyblAAAAHElEQVQI12P4//8/w38GIAXDIBKE0DHxgljNBAAO9TXL0Y4OHwAAAABJRU5ErkJggg=='), isFalse);
    });
