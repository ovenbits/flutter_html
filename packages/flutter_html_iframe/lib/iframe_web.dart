...
CustomRender iframeRender({NavigationDelegate? navigationDelegate}) => CustomRender.widget(widget: (context, buildChildren) {
      final html.IFrameElement iframe = html.IFrameElement()
        ..width = (double.tryParse(context.tree.element?.attributes['width'] ?? "") ?? (double.tryParse(context.tree.element?.attributes['height'] ?? "") ?? 150) * 2).toString()
        ..height = (double.tryParse(context.tree.element?.attributes['height'] ?? "") ?? (double.tryParse(context.tree.element?.attributes['width'] ?? "") ?? 300) / 2).toString()
